...
var brace = require('brace-expansion')
...
module.exports = function (str, cb) {
  ...
  zones.forEach(fillInTheBlanks)
  zones.forEach(expandBraces)
  cb(null, zones)
}
...
function expandBraces (zone) {
  zone.records = zone.records.reduce(function (records, r) {
    var expanded = brace(r.data)
    if (expanded.length < 2) {
      records.push(r)
      return records
    }
    return records.concat(expanded.map(function (v) {
      var record = JSON.parse(JSON.stringify(r))
      record.data = v
      return record
    }))
  }, [])
